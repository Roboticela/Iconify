...
Iconify automatically generates SHA256 checksums for all build artifacts. No certificates required!
...
```powershell
# Verify a single file
$hash = (Get-FileHash iconify.exe -Algorithm SHA256).Hash
$expected = Get-Content iconify.exe.sha256
...
├── v0.1.0/
│   ├── iconify-0.1.0.dmg
│   ├── iconify-0.1.0.dmg.sha256
│   ├── iconify-0.1.0.exe
│   ├── iconify-0.1.0.exe.sha256
│   ├── iconify-0.1.0.AppImage
│   ├── iconify-0.1.0.AppImage.sha256
│   └── CHECKSUMS.txt
```
